...
const DEFAULT_PRODUCTS = [
  ...
  },
  {
    ...
    keywords: ['读文档', '整理', '国内', '长文章', '办公', '总结', 'PDF', '报告', '合同', '读书笔记', '论文', '会议记录', '提炼重点', '文件分析']
  },
  {
  ...
  },
  {
    ...
    keywords: ['画画', '设计', '插画', '壁纸', '艺术', '绘图', '作图', '配图', '头像', '封面', '海报', '商业插画', '高质量图']
  },
  {
    ...
    keywords: ['画画', '绘图', '国内', '中文', '插画', '免费', '设计', '字节', '头像', '封面', '海报', '朋友圈配图', '表情包']
  },
  {
    ...
    keywords: ['唱歌', '作词', '作曲', '音乐', '贺卡', '歌曲', '旋律', '生日歌', '情歌', 'BGM', '背景音乐', '送朋友', '纪念日', 'rap', '儿歌', '原创歌曲', '惊喜礼物']
  },
  {
  ...
  },
  {
    ...
    keywords: ['视频', '国内', '图转视频', '短视频', '快手', '动画', '免费', '动起来', '照片动画', '人物跳舞', '卡点视频', '老照片动起来']
  },
  {
    ...
    keywords: ['视频', '电影', '动画', '特效', '剪辑', '生成视频', '专业视频', '高质量视频']
  },
  {
    ...
    keywords: ['搜索', '查资料', '写论文', '事实', '新闻', '研究', '找信息', '实时搜索', '带来源', '学术搜索']
  },
  {
...
// Use Vercel Edge Config as a simple key-value store via REST API
async function getProducts() {
  try {
    const productsJson = process.env.PRODUCTS_DATA;
    if (productsJson) {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Auth check
  const password = (req.method === 'GET' ? req.query.password : req.body?.password) as string;
  ...
  // Admin POST - save via Vercel API env var update
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!projectId || !token) {
        return res.status(500).json({ error: '缺少 VERCEL_PROJECT_ID 或 VERCEL_TOKEN 环境变量' });
      }
