...
// Default products (fallback if Edge Config is empty)
const DEFAULT_PRODUCTS = [
  ...
  },
  {
    ...
    keywords: ['唱歌', '作词', '作曲', '音乐', '贺卡', '歌曲', '旋律', '生日歌']
  },
  {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // All admin operations require password
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!edgeConfigId) throw new Error('Edge Config ID not found');
      ...
      const response = await fetch(
        `https://api.vercel.com/v1/edge-config/${edgeConfigId}/items`,
        {
          ...
          body: JSON.stringify({
            items: [{ operation: 'upsert', key: 'products', value: products }]
          })
        }
      );
      ...
      return res.status(200).json({ success: true, message: `已保存 ${products.length} 个产品` });
    } catch (e: any) {
